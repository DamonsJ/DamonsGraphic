...
namespace DGraphic {
	...
	public:
		...
		inline  DBox<T>  operator +(const DBox<T> &r) {
			DBox<T> box;
			box += *this;
			box += r;
		...
		///@return this box 
		inline  DBox<T> & operator +=(const DPoint<T> &p) {
			maxPoint_ += p;
			minPoint_ += p;
	...
	public:
		...
		inline			DPoint<T>		GetMin()						const { return minPoint_; }
		/// @brief Get max point of the box
		/// @return the max point of the box
		inline			DPoint<T>		GetMax()						const { return maxPoint_; }
		...
		inline			DPoint<T>		GetCenter()						const { return (maxPoint_ + minPoint_)*0.5f; }
		/// @brief Get box extents
		/// @return the box extents
		inline			DPoint<T>		GetExtents()					const { return (maxPoint_ - minPoint_)*0.5f; }
		/// @brief Get box width
